	...
	public class FluidSliderView : View {
		...
		public void ScrollToTemperature(Scalar temperature, bool animate) {
			if (animate) {
				...
				offset = CalculateXCoordForTemperature((float)temperature.ConvertTo(temperatureUnit).amount);
				Invalidate();
			}
		}
		...
		private void ScrollToOffset(float newOffset) {
			Log.D(this, "OldOffset: " + offset + " newOffset: " + newOffset);
			offset = newOffset;
			Invalidate();
			NotifyOfScroll();
/*	
			var ytranslator = ObjectAnimator.OfFloat(this, "offset", x);

			var animator = new AnimatorSet();
			animator.SetDuration(150);
			animator.PlayTogether(ytranslator);
			animator.Start();
*/
		}
		...
		private void DrawPressureTicks(Canvas canvas) {
			...
			var maxPress = minPress + span;
			var y = window.Height() * 0.5f;

			while (minPress < maxPressure) {
				// Draw the big tick
				var x = CalculateXCoordForTemperature((float)ptChart.GetTemperature(pressureUnit.OfScalar(minPress)).ConvertTo(temperatureUnit).amount);
				...
				for (int i = 1; i < subticks; i++) {
					var fx = CalculateXCoordForTemperature((float)ptChart.GetTemperature(pressureUnit.OfScalar(minPress + st * i)).ConvertTo(temperatureUnit).amount);
		...
		private void DrawTemperatureTicks(Canvas canvas) {
			...
			var minPress = (int)Math.Round(minPressure);
			var maxPress = (int)Math.Round(maxPressure);
...
//			temperaturePaint.TextSize = MeasuredHeight * TEXT_SIZE;
    ...
    private int CalculateStartDivisibilityPoint() {
		...
		private float CalculateXCoordForTemperature(float temperature) {
			...
			var ret = window.Left + offset + (((temperature - minTemperature) / fullRange) * width);
			return ret;
		}
		...
		private float CalculateOffsetFromTemperature(float temperature) {
			...
			return width / 2 - (((temperature - minTemperature) / fullRange) * width);
		}
...
/*
		public class GestureDetector : GestureDetector.SimpleOnGestureListener {
			override 
		}
*/
	}
