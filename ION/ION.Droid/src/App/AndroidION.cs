namespace ION.Droid.App {
  ...
  public class AndroidION : BaseAndroidION {
      ...
      try {
        ...
        foreach (var name in fluidManager.GetAvailableFluidNames()) {
          try {
            ...
            file = CreateFluidFileFromFluid(fluid, step);
            if (file.size > size) {
              size = file.size;
              fluidName = name;
            }
            fluidCount++;
            rowCount += file.tdew.Length;
            Log.D(this, "||[FORECAST]: " + name + " has " + file.tdew.Length + " rows");
          } catch (Exception ee) {
            Log.E(this, "Failed to create file from fluid: " + name, ee);
          }
        }

        Log.D(this, "||||||||||||| Out of " + fluidCount + " fluid, the max size is " + size + " bytes and is from fluid: " + fluidName + " and the average is row count is: " + (rowCount / fluidCount));
    ...
    private FluidFile CreateFluidFileFromFluid(Fluid fluid, Scalar pressureStep) {
			var psi = Units.Pressure.PSIG;
			var kpa = Units.Pressure.KILOPASCAL;
