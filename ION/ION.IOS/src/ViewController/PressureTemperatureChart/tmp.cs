...
namespace ION.IOS.ViewController.PressureTemperatureChart {
  ...
  public partial class PTChartViewControllerr : BaseIONViewController {
    ...
    public Sensor pressureSensor {
      ...
      set {
        ...
        Log.D(this, "pressure sensor is set to " + __pressureSensor);
    ...
    public override void ViewDidLoad() {
      ...
      buttonPressureUnit.SetTitle(__pressureSensor.unit.ToString(), UIControlState.Normal);
      buttonTemperatureUnit.SetTitle(__temperatureSensor.unit.ToString(), UIControlState.Normal);
      ...
      if (initialManifold != null) {
        ...
        ptChart = PTChart.New(ion, Fluid.EState.Dew, ion.fluidManager.lastUsedFluid);
        pressureSensor = null;
        temperatureSensor = null;
        ClearPressureInput();
        ClearTemperatureInput();
    ...
    private void UpdatePressureDisplay() {
      Log.D(this, "pressure at time of display");
    ...
    private void UpdateTemperatureWithCurrentPressure() {
      if (!(temperatureSensor is GaugeDeviceSensor) && !editTemperature.IsEditing) {
        ...
        if (ptChart.IsPressureAboveBounds(pressureSensor.measurement)) {
          if (pressureSensor.isEditable) {
            pressureSensor.measurement = ptChart.fluid.GetMaximumPressure(ptChart.state);
            validated = true;
          }
        } else if (ptChart.IsPressureBelowBounds(pressureSensor.measurement)) {
          if (pressureSensor.isEditable) {
            pressureSensor.measurement = ptChart.fluid.GetMinimumPressure(ptChart.state);
            validated = true;
          }
        } else {
          validated = true;
