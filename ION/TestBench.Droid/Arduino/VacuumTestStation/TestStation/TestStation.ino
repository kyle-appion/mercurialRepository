...
void setup() {
  ...
  btController = new BluetoothController(BLUEFRUIT_SPI_CS, BLUEFRUIT_SPI_IRQ, BLUEFRUIT_SPI_RST, "CV17G001");
  vrc = new Vrc();
  ...
  driver = new TestDriver(intake, exhaust, vrc);
...
*/
    ...
    if (timeSinceLastWrite >= BLUETOOTH_WRITE_INTERVAL) {
      ...
      f32 cep = exhaust->GetCurrentPositionAsDegrees();
      ...
      memcpy(buffer + (offset += sizeof(cip)), &cep, sizeof(cep));
