...
namespace ION.Core.Net {
	...
	public class WebPayload {
		...
		public async Task<HttpResponseMessage> getSession(ObservableCollection<int> sessionList){
      ...
      foreach (var num in sessionList) {
        paramList.Add('"' + num.ToString() + '"');
			...
			var response = await UploadSession(jsonPayload,true);
			return response;
		}
		/// <summary>
		...
		public async Task<HttpResponseMessage> UploadSession(string json, bool isJson = true){
    ...
    public async Task<HttpResponseMessage> RegisterUser(string firstName, string password, string lastName, string email){
    	await Task.Delay(TimeSpan.FromMilliseconds(1)); 
			...
			if(retrieved == null){
				...
				foreach (var con in dManager) {
					...
					if(remoteDManager.knownDevices.Contains(manualDevice)){
						...
						for(int i = 0; i < deserializedToken.sensors.Length;i++){
							newDevice.sensors[i].RemoteForceSetMeasurement(new Scalar(UnitLookup.GetUnit(deserializedToken.sensors[i].unit),deserializedToken.sensors[i].measurement));
						}
					...
					foreach(var device in remoteDManager.knownDevices){
						...
						if(gDevice.serialNumber.rawSerial == deserializedToken.serialNumber){
							...
							if(remoteAnalyzer.sensorList.Contains(gDevice.sensors[0]) && gDevice.sensors[0].type == UnitLookup.GetSensorTypeFromCode(deserializedToken.unit)){
								gDevice.sensors[0].RemoteForceSetMeasurement(new Scalar(UnitLookup.GetUnit(deserializedToken.unit),deserializedToken.measurement));
								...
								gDevice.sensors[1].RemoteForceSetMeasurement(new Scalar(UnitLookup.GetUnit(deserializedToken.unit),deserializedToken.measurement));
							}  else {
								if(gDevice.serialNumber.rawSerial == deserializedToken.serialNumber){
				...
				foreach(var con in wManager){
					...
					if(existing != 99){
						...
						updateManifold.primarySensor.RemoteForceSetMeasurement(new Scalar(UnitLookup.GetUnit(deserializedToken.unit),deserializedToken.measurement));
	...
	public async Task<HttpResponseMessage> updatePassword(string newPword){
		await Task.Delay(TimeSpan.FromMilliseconds(1));
