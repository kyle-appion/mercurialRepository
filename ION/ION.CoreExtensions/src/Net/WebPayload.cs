...
namespace ION.Core.Net {
		...
		if(uploadWorkbench != null && uploadWorkbench.manifolds != null){
			...
			foreach(var manifold in uploadWorkbench.manifolds){
				...
				if(manifold.secondarySensor != null && manifold.secondarySensor is GaugeDeviceSensor){
					...
					layoutJson += "\"link\":\"" +deviceSensor.device.serialNumber.rawSerial + "\",";
				} else {
					layoutJson += "\"link\":\"null\",";
				}
				layoutJson += "\"sub\":[";
				var subcount = 1;
	...
	public async Task DownloadLayouts(string ID){
      ...
      var formContent = new FormUrlEncodedContent(new[]
          {
			...
			if(retrieved == null){
				...
				Console.WriteLine("Low subviews ");
				foreach(var suv in deserializedLowHigh.lowSubviews){
					Console.WriteLine(suv);
				}
				Console.WriteLine(Environment.NewLine);
				Console.WriteLine("high subviews " + deserializedLowHigh.highSubviews);
				foreach(var suv in deserializedLowHigh.highSubviews){
					Console.WriteLine(suv);
				}
				...
				//Console.WriteLine(wManager.ToString());
				...
				foreach(var con in wManager){
					...
					if(existing != 99){
						...
						if(UnitLookup.GetSensorTypeFromCode(deserializedToken.unit) == updateManifold.primarySensor.type){
							updateManifold.primarySensor.ForceSetMeasurement(new Scalar(UnitLookup.GetUnit(deserializedToken.unit),deserializedToken.measurement));
						} else if (updateManifold.secondarySensor != null && UnitLookup.GetSensorTypeFromCode(deserializedToken.unit) == updateManifold.secondarySensor.type) {
							updateManifold.secondarySensor.ForceSetMeasurement(new Scalar(UnitLookup.GetUnit(deserializedToken.unit),deserializedToken.measurement));
						}
						...
						if(deserializedToken.linkedSerial != "null" ){
							if(updateManifold.secondarySensor != null){
								...
								if(checkSensor.device.serialNumber.rawSerial != deserializedToken.linkedSerial){
									...
									///create an idevice
									var linkedDevice = remoteDManager.CreateDevice(linkedSerial,Devices.Protocols.EProtocolVersion.V4,true);
									///locate idevice in known devices
									var linkedIndex = remoteDManager.knownDevices.IndexOf(linkedDevice);
									var knownLinked = remoteDManager.knownDevices[linkedIndex] as GaugeDevice;
									///get unit type of parent sensor
									var parentType = updateManifold.primarySensor.type;
									///check linked type against parent type
									if(parentType == ESensorType.Pressure){
										for(int i = 0; i < knownLinked.sensorCount; i++){
											if(knownLinked.sensors[i].type == ESensorType.Temperature){
												updateManifold.SetSecondarySensor(knownLinked.sensors[i]);
											}
										}
									} else {
										for(int i = 0; i < knownLinked.sensorCount; i++){
											if(knownLinked.sensors[i].type == ESensorType.Pressure){
												updateManifold.SetSecondarySensor(knownLinked.sensors[i]);
											}
										}									
									}
									...
									///find the sensor in known devices that matches the linked serial and is the opposite unit type of the parent
									///create an iserial
									var linkedSerial = SerialNumberExtensions.ParseSerialNumber(deserializedToken.linkedSerial);
									///create an idevice
									var linkedDevice = remoteDManager.CreateDevice(linkedSerial,Devices.Protocols.EProtocolVersion.V4,true);
	...
	/// </summary>
	public async void StartLayoutDownload(string viewingID, string userID){
		  ...
		  var timeDifference = DateTime.Now.Subtract(startedViewing).Minutes;
			...
			if(timeDifference < 360){
				if(!string.IsNullOrEmpty(userID)){
					await DownloadLayouts(viewingID);
